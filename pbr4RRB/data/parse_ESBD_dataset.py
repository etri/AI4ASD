...
action_cls_mapping ={'armflapping':'ArmFlapping', 'spinning':'Spinning', 'headbanging':'HeadBanging', 'jumping':'Jumping', 'toyplaying':'ToyPlaying', 'etc':'etc', 'handaction':'HandAction'}
...
for i in range(len(sub_folder_list)):
    gt_list += [os.path.join(gt_dir+sub_folder_list[i], f) for f in sorted(os.listdir(gt_dir+sub_folder_list[i])) if f.split(".")[-1] == "xml"]


for i in range(len(gt_list)):

    file_name = gt_list[i].split('/')
    action_cls = file_name[-2]
    file_name = file_name[-1].split('.')
    file_name = file_name[0]

    video_name = file_dir + action_cls + '/' + file_name + '.mp4'
    ...
    folder_name_period = file_dir + action_cls + '_per_frame_periodicity_colorful'

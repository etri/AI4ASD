...
import numpy as np
import random
from libFD.retinaface.utils.box_utils import matrix_iof
...
def _crop(image, boxes, labels, landm, img_dim):
    ...
    for _ in range(250):
        ...
        if width == w:
            ...
            _l = random.randrange(width - w)
        if height == h:
            t = 0
        else:
            t = random.randrange(height - h)
        roi = np.array((l, t, _l + w, t + h))
...
def _resize_subtract_mean(image, insize, rgb_mean):
    ...
    """_resize_subtract_mean function
